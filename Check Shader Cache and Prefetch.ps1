...
foreach ($folder in $folders) {
    if (Test-Path $folder) {
        ...
        Write-Host "$folder - Tamanho: $sizeMB MB"
        $totalSize += $size
    } else {
...
# Calcula e exibe o tamanho total em diferentes unidades
$totalSizeMB = "{0:N2}" -f ($totalSize / 1MB)
$totalSizeGB = "{0:N2}" -f ($totalSize / 1GB)

Write-Host ""
Write-Host "============================================="
Write-Host "TAMANHO TOTAL DE TODOS OS CACHES DE SHADER E PREFETCH:"
